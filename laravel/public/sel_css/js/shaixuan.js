...
oSelectList.onclick = function(e, a) {
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
            ...
            if (trim(prev(parent).innerHTML) == '房屋价格') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                for (var i = 0; i < aRadio.length; i++) {
                    aRadio[i].checked = false;
                }
                isCusPrice = false;
    ...
    if(a && a == 2) {
       ...
       if (radioVal) infor += '<div class=\"selectedInfor selectedShow\"><span>房屋价格</span><label>' + radioVal + '</label><em p="2"></em></div>';
    }
